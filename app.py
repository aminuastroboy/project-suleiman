...
st.set_page_config(page_title='CBT App Final', layout='wide')
utils.init_db()
...
st.title('CBT WebApp - Final Build')
...
if st.session_state['page'] == 'home':
    st.header('Welcome to the CBT System (Final)')
    st.write('Use the top buttons to navigate. Admin: ADMIN001 / admin123')
...
if st.session_state['page'] == 'admin_dashboard':
    if not st.session_state['user'] or st.session_state['user'].get('role')!='admin':
        ...
        for s in utils.list_students():
            st.write(f"{s['reg_no']} - {s['name']} - face: {'Yes' if s['face_embedding'] else 'No'}")
            if st.button('Delete '+s['reg_no']):
                conn = utils.get_conn(); cur = conn.cursor()
                cur.execute('DELETE FROM users WHERE id=?', (s['id'],)); conn.commit(); conn.close(); st.experimental_rerun()
...
if st.session_state['page'] == 'student_dashboard':
    if not st.session_state['user']:
        ...
        if st.button('Start Exam'):
            # require re-auth if face exists
            row = utils.get_user_by_reg(user['reg_no'])
        ...
        if not qs:
            ...
            with c3:
                if st.button('Submit Exam'):
                    ...
                    conn = utils.get_conn(); cur = conn.cursor()
                    cur.execute('INSERT INTO attempts (user_id, score, total, created_at) VALUES (?,?,?,?)', (st.session_state['user']['id'], 0, total, datetime.utcnow().isoformat()))
